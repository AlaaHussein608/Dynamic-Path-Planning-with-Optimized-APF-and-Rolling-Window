...
class APF:
    ...
    def dynamics(self):
        ...
        while(1):
            if j==14 or (math.isclose(self.q_robot[0],self.q_goal[0],abs_tol=self.step_size) and math.isclose(self.q_robot[1],self.q_goal[1],abs_tol=self.step_size)):
                self.q_goal = self.select_sub_target(global_goal)
                j=0
            i += 1
            j+=1
...
side = [[15,15],[15,15],[10,10],[10,10],[10,10],[15,15],[10,10]]
obstacles = [[10,10],[20,20],[50,50],[30,30],[70,70],[80,90],[60,30]]


abf = APF([0,0], [100,100], obstacles, side, 0.5, 1000, 0.5, 2)
